...
class Setting extends BaseController
{
    ...
    {
        try {
            if (!check_role(['administrator', 'admin_ulp', 'inspeksi', 'supervisor_ulp', 'supervisor_up3'])) {
                if ($this->request->isAJAX()) {
                    return $this->response->setJSON(['success' => false, 'message' => 'Akses ditolak. Anda tidak memiliki wewenang mengedit motivasi.']);
                }
                return redirect()->to(site_url('dashboard'))->with('error', 'Akses ditolak.');
            }

            $session = session();
            $userName = $session->get('user_name') ?: ($session->get('nama_pegawai') ?: 'Administrator');
            ...
            if ($this->request->isAJAX()) {
                return $this->response->setJSON([
                    ...
                    'csrfHash'     => csrf_hash(),
                    'updated_by'   => $userName,
                    'updated_at'   => function_exists('indo_datetime') ? indo_datetime(date('Y-m-d H:i:s')) : date('d-m-Y H:i')
                ]);
            }
            ...
            log_message('error', '[Setting::updateAnnouncement] Exception: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
            if ($this->request->isAJAX()) {
                return $this->response->setJSON([
                    'success' => false,
                    'message' => 'Gagal menyimpan motivasi: ' . $e->getMessage()
                ]);
            }
            return redirect()->back()->with('error', 'Gagal menyimpan motivasi: ' . $e->getMessage());
        }
    }
}
