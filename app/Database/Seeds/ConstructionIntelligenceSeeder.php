...
/**
 * Enterprise Seeder for CR-06 Construction Intelligence (9-Sheet Knowledge Blueprint)
 * Atomic, Idempotent, and Fail-Safe.
 */
class ConstructionIntelligenceSeeder extends Seeder
{
    public function run()
    {
        ...
        try {
            ...
            $gttProgram = $inspService->registerInspectionProgram([
                'program_code'   => 'INSP-GTT-LOAD-MEASURE',
                'nama_pekerjaan' => 'Pengukuran Beban Gardu Tiang (GTT)',
                'asset_domain'   => 'TRAFO',
                'inspection_type'=> 'LOAD_MEASUREMENT',
                'executor_type'  => 'HAR_GARDU',
            ]);
            ...
            $db->transRollback();
            log_message('error', '[ConstructionIntelligenceSeeder] Seeder failed: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
            throw new \RuntimeException('ConstructionIntelligenceSeeder failed: ' . $e->getMessage() . ' at ' . $e->getFile() . ':' . $e->getLine(), 0, $e);
        }
    }
}
