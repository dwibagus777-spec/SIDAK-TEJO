...
<?= $this->section('title') ?>Tambah Eviden Trafo<?= $this->endSection() ?>
<?= $this->section('page_title') ?>Input Eviden Trafo<?= $this->endSection() ?>

<?= $this->section('breadcrumb') ?>
<li class="breadcrumb-item"><a href="<?= site_url('eviden/trafo') ?>">Eviden Trafo</a></li>
        ...
        <div class="card card-modern">
            <div class="card-header card-header-modern">
                <h3 class="card-title text-white font-weight-bold">
                    <i class="fas fa-plus-circle mr-2"></i> Formulir Eviden Pemeliharaan Trafo
                </h3>
            </div>
            
            <form action="<?= site_url('eviden/trafo/store') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="card-body">
                    ...
                    <div class="form-group mt-2">
                        ...
                        <textarea id="keterangan" name="keterangan" class="form-control" rows="3" placeholder="Tuliskan rincian hasil pemeliharaan trafo..." required><?= old('keterangan') ?></textarea>
                    </div>

                    <!-- FOTO EVIDEN GRID (TOTAL 16 FOTO) -->
                    <h5 class="mt-4 border-top pt-3 font-weight-bold text-info"><i class="fas fa-camera mr-1"></i> Upload Foto Eviden (Total 16 Foto)</h5>
                    
                    <!-- 1. Foto Nameplate (1 Foto) -->
                    <div class="card card-outline card-info mt-3 shadow-none bg-light">
                        <div class="card-header py-2"><h6 class="mb-0 font-weight-bold text-primary">Foto Nameplate (1 Foto)</h6></div>
                        <div class="card-body py-2">
                            <div class="row">
                                <div class="col-md-4 form-group mb-2">
                                    <label class="small font-weight-bold">Foto Nameplate</label>
                    ...
                    <!-- 2. Foto PHBTR (2 Foto) -->
                    <div class="card card-outline card-info mt-3 shadow-none bg-light">
                        <div class="card-header py-2"><h6 class="mb-0 font-weight-bold text-primary">Foto PHBTR (2 Foto)</h6></div>
                        <div class="card-body py-2">
                            <div class="row">
                                <?php for($i = 1; $i <= 2; $i++): ?>
                                <div class="col-md-6 form-group mb-2">
                                    <label class="small font-weight-bold">PHBTR <?= $i ?></label>
                                    <input type="file" name="foto_phbtr[]" class="form-control-file border p-1 rounded bg-white" accept="image/*">
                                </div>
                                <?php endfor; ?>
                            </div>
                        </div>
                    ...
                    <!-- 3. Foto Trafo (2 Foto) -->
                    <div class="card card-outline card-info mt-3 shadow-none bg-light">
                        <div class="card-header py-2"><h6 class="mb-0 font-weight-bold text-primary">Foto Trafo (2 Foto)</h6></div>
                        <div class="card-body py-2">
                            <div class="row">
                                <?php for($i = 1; $i <= 2; $i++): ?>
                                <div class="col-md-6 form-group mb-2">
                                    <label class="small font-weight-bold">Foto Trafo <?= $i ?></label>
                                    <input type="file" name="foto_trafo[]" class="form-control-file border p-1 rounded bg-white" accept="image/*">
                                </div>
                                <?php endfor; ?>
                            </div>
                        </div>
                    ...
                    <!-- 5. Foto Perbaikan/Pergantian (4 Foto) -->
                    ...
                    <!-- 6. Foto Hasil Megger (4 Foto) -->
                    <div class="card card-outline card-warning mt-3 shadow-none bg-light">
                        <div class="card-header py-2"><h6 class="mb-0 font-weight-bold text-warning">Foto Hasil Megger (4 Foto)</h6></div>
                        <div class="card-body py-2">
                            <div class="row">
                                <?php for($i = 1; $i <= 4; $i++): ?>
                                <div class="col-md-3 form-group mb-2">
                                    <label class="small font-weight-bold">Megger <?= $i ?></label>
                                    <input type="file" name="foto_megger[]" class="form-control-file border p-1 rounded bg-white" accept="image/*">
                                </div>
                                <?php endfor; ?>
                            </div>
                        </div>
                ...
                <div class="card-footer bg-white border-top text-right">
                    <a href="<?= site_url('eviden/trafo') ?>" class="btn btn-secondary font-weight-bold px-4 mr-2">Batal</a>
