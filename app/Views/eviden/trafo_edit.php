...
<?= $this->section('title') ?>Ubah Eviden Trafo<?= $this->endSection() ?>
<?= $this->section('page_title') ?>Edit Eviden Trafo<?= $this->endSection() ?>
        ...
        <div class="card card-modern">
            <div class="card-header card-header-modern">
                <h3 class="card-title text-white font-weight-bold">
                    <i class="fas fa-edit mr-2"></i> Edit Formulir Eviden Pemeliharaan Trafo
                </h3>
            </div>
            
            <form action="<?= site_url('eviden/trafo/update/' . $trafo['id_trafo']) ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <div class="card-body">
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <select id="id_penyulang" name="id_penyulang" class="form-control select2" required>
                                ...
                                <?php foreach ($penyulangs as $p): ?>
                                    <option value="<?= $p['id'] ?>" <?= (old('id_penyulang', $trafo['id_penyulang']) == $p['id']) ? 'selected' : '' ?>><?= esc($p['nama_penyulang']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <select id="id_section" name="id_section" class="form-control select2" required>
                                ...
                                <?php foreach ($sections as $s): ?>
                                    <option value="<?= $s['id'] ?>" <?= (old('id_section', $trafo['id_section']) == $s['id']) ? 'selected' : '' ?>><?= esc($s['nama_section']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row mt-2">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="nama_gardu" name="nama_gardu" class="form-control" value="<?= esc(old('nama_gardu', $trafo['nama_gardu'])) ?>" required>
                        </div>

                        <!-- Tanggal Input -->
                        <div class="col-md-6 form-group">
                            <label for="tgl_input" class="font-weight-bold">Tanggal Input <span class="text-danger">*</span></label>
                            <input type="date" id="tgl_input" name="tgl_input" class="form-control" value="<?= esc(old('tgl_input', $trafo['tgl_input'])) ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group mt-2">
                        ...
                        <textarea id="keterangan" name="keterangan" class="form-control" rows="3" required><?= esc(old('keterangan', $trafo['keterangan'])) ?></textarea>
                    </div>
                    ...
                    <div class="row mt-2">
                        ...
                        <!-- 2. Foto PHBTR -->
                        <div class="col-md-6 form-group">
                            <label class="small font-weight-bold">Tambah Foto PHBTR</label>
                            <input type="file" name="foto_phbtr[]" class="form-control-file border p-1 rounded bg-white" accept="image/*" multiple>
                        </div>
                    </div>
                    ...
                    <div class="row mt-2">
                        <!-- 3. Foto Trafo -->
                        <div class="col-md-6 form-group">
                            <label class="small font-weight-bold">Tambah Foto Trafo</label>
                            <input type="file" name="foto_trafo[]" class="form-control-file border p-1 rounded bg-white" accept="image/*" multiple>
                        </div>
                    ...
                    <div class="row mt-2">
                        ...
                        <!-- 6. Foto Hasil Megger -->
                        <div class="col-md-6 form-group">
                            <label class="small font-weight-bold">Tambah Foto Hasil Megger</label>
                            <input type="file" name="foto_megger[]" class="form-control-file border p-1 rounded bg-white" accept="image/*" multiple>
                        </div>
                    </div>
