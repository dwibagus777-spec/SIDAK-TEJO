...
<?= $this->section('title') ?>Eviden Trafo<?= $this->endSection() ?>
<?= $this->section('page_title') ?>Eviden Lapangan Trafo<?= $this->endSection() ?>

<?= $this->section('breadcrumb') ?>
<li class="breadcrumb-item active">Eviden Trafo</li>
<?= $this->endSection() ?>
...
<!-- PANEL FILTER DATA EVIDEN TRAFO -->
<div class="row mb-3">
    <div class="col-12">
        <div class="card card-outline card-info shadow-sm">
            ...
            <div class="card-body py-3">
                <form method="GET" action="<?= site_url('eviden/trafo') ?>" id="form-filter-eviden">
                    <div class="row">
                        <!-- Filter ULP -->
                        ...
                        <div class="col-md-2 form-group mb-2 d-flex align-items-end" style="gap: 6px;">
                            ...
                            <a href="<?= site_url('eviden/trafo') ?>" class="btn btn-secondary btn-sm btn-block"><i class="fas fa-undo mr-1"></i> Reset</a>
                        </div>
                    </div>
                </form>
            </div>
...
<div class="row">
    <div class="col-12">
        ...
        <form id="form-bulk-action" method="POST" style="display:none;">
            <?= csrf_field() ?>
            <input type="hidden" name="kategori" value="TRAFO">
        ...
        <div class="card card-modern">
            <div class="card-header card-header-modern d-flex align-items-center justify-content-between">
                <h3 class="card-title text-white font-weight-bold">
                    <i class="fas fa-bolt mr-2"></i> Data Eviden Pemeliharaan Trafo
                </h3>
                <div class="d-flex" style="gap: 8px;">
                    ...
                    <a href="<?= site_url('eviden/export-trafo?' . http_build_query(service('request')->getGet())) ?>" class="btn btn-sm btn-success font-weight-bold"><i class="fas fa-download mr-1"></i> Download CSV</a>
                    <a href="<?= site_url('eviden/trafo/create') ?>" class="btn btn-sm btn-light font-weight-bold text-primary">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="table-eviden-trafo" class="table table-hover table-modern text-center">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php $no = 1; foreach ($dataList as $row): ?>
                                <tr>
                                    <td><input type="checkbox" class="check-item" value="<?= $row['id_trafo'] ?>"></td>
                                    <td><?= $no++ ?></td>
                                    ...
                                    <td>
                                        <?php if ($row['foto_count'] > 0): ?>
                                            <button type="button" class="btn btn-sm btn-info btn-view-fotos" data-parent-id="<?= $row['id_trafo'] ?>" data-kategori="TRAFO" title="Lihat Galeri Foto">
                                    ...
                                    <td>
                                        <a href="<?= site_url('eviden/trafo/edit/' . $row['id_trafo']) ?>" class="btn btn-action btn-detail-action mr-1" title="Ubah">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="javascript:void(0)" onclick="confirmDelete(<?= $row['id_trafo'] ?>)" class="btn btn-action btn-delete-action" title="Hapus">
...
<script>
    $(function () {
        // Init DataTable
        const table = $('#table-eviden-trafo').DataTable({
            "responsive": true,
            "autoWidth": false,
            "columnDefs": [
        ...
        $('#table-eviden-trafo tbody').on('change', 'input[type="checkbox"]', function() {
            if (!this.checked) {
    ...
    function confirmDelete(id) {
        Swal.fire({
            ...
            text: "Data eviden trafo beserta seluruh berkas foto terkait akan dihapus secara permanen!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= site_url('eviden/trafo/delete/') ?>' + id,
                    type: 'GET',
                    dataType: 'JSON',
