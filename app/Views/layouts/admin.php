...
<body class="<?= $isMobileMode ? 'is-mobile-app' : '' ?>">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ── ENTERPRISE CINEMATIC LOGOUT INTERCEPTOR ──
        var logoutLinks = document.querySelectorAll('a[href*="logout"]');
        logoutLinks.forEach(function(link) {
            link.addEventListener('click', function(e) {
                if (this.dataset.loggingOut || document.body.classList.contains('is-logging-out')) return;
                
                this.dataset.loggingOut = 'true';
                document.body.classList.add('is-logging-out');
                this.style.pointerEvents = 'none';

                var overlay = document.getElementById('cinematic-logout-overlay');
                if (overlay) {
    ...
    .cinematic-reveal .cinematic-panel-right {
        ...
        height: 100%;
        background: #ffffff;
        border-left: 1px solid rgba(0, 181, 184, 0.15);
        transition: transform 850ms cubic-bezier(0.16, 1, 0.3, 1);
        will-change: transform;
        transform: translateX(0);
    }
    ...
    .cinematic-logout-overlay.active {
        pointer-events: auto;
    }
    .cinematic-logout-overlay .cinematic-panel {
        width: 50vw;
        height: 100vh;
    ...
    .cinematic-logout-overlay .cinematic-panel-left {
        transform: translateX(-100%);
        border-right: 1px solid rgba(0, 181, 184, 0.2);
    }
    .cinematic-logout-overlay .cinematic-panel-right {
        transform: translateX(100%);
        border-left: 1px solid rgba(0, 181, 184, 0.2);
    }
    .cinematic-logout-overlay.active .cinematic-panel-left {
        transform: translateX(0);
    }
    .cinematic-logout-overlay.active .cinematic-panel-right {
        transform: translateX(0);
    }

    .cinematic-logout-brand {
        position: absolute;
        top: 50%;
        left: 50%;
    ...
    .cinematic-logout-overlay.active .cinematic-logout-brand {
        opacity: 1;
    }
